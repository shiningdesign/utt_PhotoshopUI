'''
based on Univeral Tool Template v007
...
import sys
import module_photoshop
reload(module_photoshop)
########################################
...
class PhotoshopUI(QtGui.QMainWindow):
#class PhotoshopUI(QtGui.QDialog):
    ...
    def setupWin(self):
        self.setWindowTitle("PhotoshopUI" + " - v" + self.version) 
        self.setGeometry(300, 300, 800, 600)
        # win icon setup
        path = os.path.join(os.path.dirname(self.location),'icons','PhotoshopUI.png')
    ...
    def process_action(self): # (optional)
        print("Process ....")
        my_cmd = str(self.uiList['source_txtEdit'].text())
        my_result = module_photoshop.ps_cmd(my_cmd)
        print("Update Result")
        self.uiList['result_txtEdit'].setText(my_result)
...
def main(mode=0):
    ...
    global single_PhotoshopUI
    if single_PhotoshopUI is None:
        single_PhotoshopUI = PhotoshopUI(parentWin, mode) # extra note: in Maya () for no parent; (parentWin,0) for extra mode input
    single_PhotoshopUI.show()
    
    if deskMode == 1:
        sys.exit(app.exec_())
    
    # example: show ui stored
    # print(single_PhotoshopUI.uiList.keys())
    return single_PhotoshopUI
...
def main(mode=0):
    ...
    ui = PhotoshopUI(parentWin) # extra note: in Maya () for no parent; (parentWin,mode) for extra mode input
    ui.show()
    # example: show ui stored
